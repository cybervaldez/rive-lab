...
export const recipes: Recipe[] = [
  ...
  },
  {
    ...
    contract: [
      { xstate: 'context.bindings', rive: 'ViewModel property bindings (Object)' },
      { xstate: 'context.activeInputs', rive: 'ViewModel property activeInputs (List)' },
      { xstate: 'context.activeTab', rive: 'ViewModel property activeTab (String)' },
      { xstate: 'context.sources', rive: 'ViewModel property sources (Object)' },
      { xstate: 'event: KEY_DOWN', rive: 'Trigger KEY_DOWN' },
      { xstate: 'event: KEY_UP', rive: 'Trigger KEY_UP' },
      { xstate: 'event: TRIGGER_INPUT', rive: 'Trigger TRIGGER_INPUT' },
      { xstate: 'event: SET_TAB', rive: 'Trigger SET_TAB' },
      { xstate: 'event: reset', rive: 'Trigger reset' },
      { xstate: 'state: active', rive: 'State: active' },
      { xstate: 'state: {configuring: idle}', rive: 'State: configuring.idle' },
      { xstate: 'state: {configuring: listening}', rive: 'State: configuring.listening' },
    ...
    events: [
      { name: 'bindings', direction: 'in', type: 'Object Input', description: 'Action-to-key mapping' },
      { name: 'activeInputs', direction: 'in', type: 'List Input', description: 'Currently active actions' },
      { name: 'activeTab', direction: 'in', type: 'String Input', description: 'Active control room tab' },
      { name: 'sources', direction: 'in', type: 'Object Input', description: 'Input source states' },
      { name: 'KEY_DOWN', direction: 'in', type: 'Trigger', description: 'Key pressed (held mode)' },
      { name: 'KEY_UP', direction: 'in', type: 'Trigger', description: 'Key released (held mode)' },
      { name: 'TRIGGER_INPUT', direction: 'in', type: 'Trigger', description: 'Fire-and-forget input from voice/face/API' },
      { name: 'SET_TAB', direction: 'in', type: 'Trigger', description: 'Switch control room tab' },
      { name: 'reset', direction: 'in', type: 'Trigger', description: 'Reset all state to defaults' },
    ],
    instruct: [
      {
        step: 'Create ViewModel',
        detail: 'Add a ViewModel named StreamOverlayVM to the artboard. This drives both the live view overlay and the control room preview.',
        verifies: ['state:active'],
      },
      {
        step: 'Add core properties',
        detail: 'Create bindings (Object), activeInputs (List), activeTab (String), and sources (Object) properties.',
        verifies: ['context.bindings', 'context.activeInputs'],
      },
      {
        step: 'Create State Machine',
        detail: 'Name it StreamOverlaySM with states: active, configuring (idle, listening).',
        verifies: ['state:active', 'state:{configuring:idle}'],
      },
      {
        step: 'Wire input events',
        detail: 'KEY_DOWN/KEY_UP in active state update activeInputs. TRIGGER_INPUT fires and auto-releases after 200ms.',
        verifies: ['event:KEY_DOWN', 'event:KEY_UP', 'event:TRIGGER_INPUT'],
      },
      {
        step: 'Wire tab and rebind events',
        detail: 'SET_TAB switches active tab. OPEN_MAPPER/START_REBIND/CLOSE_MAPPER handle key rebinding flow.',
        verifies: ['event:SET_TAB'],
      },
    ],
    readout: [
      { label: 'state', source: 'state' },
    ...
    concept: {
      summary: 'Streamer overlay driven by multi-source inputs (keyboard, voice, face, API). The live view shows transparent effects for OBS browser source. The control room provides tabs for configuration.',
      regions: [
        { name: 'Live View', properties: ['bindings', 'activeInputs'], description: 'Transparent overlay showing action indicators — loaded in OBS as browser source' },
        { name: 'Control Room', properties: ['activeTab', 'sources', 'bindings', 'mapperOpen'], description: 'Tab-based configuration for bindings, sources, and effects' },
      ],
    },
  },
  {
